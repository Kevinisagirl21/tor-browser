...
add_task(
  async function test_remove_all_tour_notifications_through_close_button() {
    ...
    ok(
      !tourId,
      "Should not prompt tour notifications any more after closing all notifcations."
    );
    ...
    async function closeTourNotificationsOneByOne() {
      for (let i = 0; i < tourIds.length; ++i) {
        ...
        targetTourId = await getCurrentNotificationTargetTourId(
          tab.linkedBrowser
        );
        is(
          targetTourId,
          tourIds[i],
          `Should show tour notifications of ${targetTourId}`
        );
...
add_task(
  async function test_remove_all_tour_notifications_through_action_button() {
    resetOnboardingDefaultState();
    skipMuteNotificationOnFirstSession();
    ...
    ok(
      !tourId,
      "Should not prompt tour notifcations any more after taking actions on all notifcations."
    );
    BrowserTestUtils.removeTab(tab);
