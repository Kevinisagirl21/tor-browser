...
add_task(async function test_not_show_notification_for_completed_tour() {
  ...
  let targetTourId = await getCurrentNotificationTargetTourId(
    tab.linkedBrowser
  );
  is(
    targetTourId,
    lastTourId,
    "Should not show notification for completed tour"
  );
...
add_task(async function test_skip_notification_for_completed_tour() {
  ...
  is(
    targetTourId,
    tourIds[2],
    "Should skip notification for the completed 2nd tour"
  );
  BrowserTestUtils.removeTab(tab);
});
...
add_task(async function test_mute_notification_on_1st_session() {
  ...
  await waitUntilWindowIdle(tab.linkedBrowser);
  await waitUntilWindowIdle(tab.linkedBrowser);
  let promptCount = Preferences.get(
  ...
  is(
    0,
    promptCount,
    "Should not prompt tour notification during the mute duration on the 1st session"
  );

  // Test notification prompted after the mute duration on the 1st session
  let muteTime = Preferences.get(
  ...
  is(
    1,
    promptCount,
    "Should prompt tour notification after the mute duration on the 1st session"
  );
  BrowserTestUtils.removeTab(tab);
});
