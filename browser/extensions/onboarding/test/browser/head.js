...
function resetOnboardingDefaultState() {
  ...
  TOUR_IDs.forEach(id =>
    Preferences.reset(`browser.onboarding.tour.${id}.completed`)
  );
  UPDATE_TOUR_IDs.forEach(id =>
    Preferences.reset(`browser.onboarding.tour.${id}.completed`)
  );
}
    ...
    return new Promise(resolve => {
      let observer = new content.MutationObserver(mutations => {
        mutations.forEach(mutation => {
          let overlay = Array.from(mutation.addedNodes).find(
            node => node.id == "onboarding-overlay"
          );
          if (overlay) {
            observer.disconnect();
            ok(true, "Should load onboarding overlay");
  ...
  return BrowserTestUtils.waitForCondition(
    () =>
      ContentTask.spawn(browser, {}, () =>
        content.document
          .querySelector("#onboarding-overlay")
          .classList.contains("onboarding-opened")
      ),
...
function promiseTourNotificationOpened(browser) {
  function isOpened() {
    ...
    return new Promise(resolve => {
      let observer = new content.MutationObserver(mutations => {
        mutations.forEach(mutation => {
          let bar = Array.from(mutation.addedNodes).find(
            node => node.id == "onboarding-notification-bar"
          );
...
function promiseTourNotificationClosed(browser) {
  let condition = () => {
    return ContentTask.spawn(browser, {}, function() {
      return new Promise(resolve => {
        let bar = content.document.querySelector(
...
function assertOverlaySemantics(browser) {
  return ContentTask.spawn(browser, {}, function() {
    ...
    is(
      dialog.tabIndex,
      "-1",
      "Dialog should be focusable but not in tab order"
    );
    is(
    ...
    is(
      doc.getElementById("onboarding-tour-list").getAttribute("role"),
      "tablist",
    ...
    items.forEach(item => {
      is(
        item.parentNode.getAttribute("role"),
        "presentation",
      ...
      is(
        item.tabIndex,
        "0",
        "Item tab index must be set for keyboard accessibility"
      );
      is(
        item.getAttribute("role"),
        "tab",
        "Item should have a tab role attribute set"
      );
...
function assertModalDialog(browser, args) {
  return ContentTask.spawn(
    browser,
    args,
    ({ keyboardFocus, visible, focusedId }) => {
      let doc = content.document;
      ...
      if (visible) {
        ...
        is(
          focusedId ? doc.getElementById(focusedId) : doc.body,
          doc.activeElement,
          `Focus should be on ${focusedId || "body"}`
        );
        is(
          keyboardFocus ? "true" : undefined,
...
function assertWatermarkIconDisplayed(browser) {
  return ContentTask.spawn(browser, {}, function() {
    let overlayButton = content.document.getElementById(
      "onboarding-overlay-button"
    );
    ok(
      overlayButton.classList.contains("onboarding-watermark"),
      "Should display the watermark onboarding icon"
    );
  });
}
