...
function assertTourCompleted(tourId, expectComplete, browser) {
  return ContentTask.spawn(browser, { tourId, expectComplete }, function(args) {
    let item = content.document.querySelector(
      `#${args.tourId}.onboarding-tour-item`
    );
    ...
    if (args.expectComplete) {
      ok(
        item.classList.contains("onboarding-complete"),
        `Should set the complete #${args.tourId} tour with the complete style`
      );
      ok(completedText, "Text label should be present for a completed item");
      is(
        completedText.id,
        completedTextId,
        "Text label node should have a unique id"
      );
      ok(
        completedText.getAttribute("aria-label"),
        "Text label node should have an aria-label attribute set"
      );
      is(
      ...
      ok(
        !item.classList.contains("onboarding-complete"),
        `Should not set the incomplete #${args.tourId} tour with the complete style`
      );
      ok(
        !completedText,
        "Text label should not be present for an incomplete item"
      );
      ok(
        !item.hasAttribute("aria-describedby"),
        "Incomplete item should not have aria-describedby attribute set"
      );
    }
  });
}
...
add_task(async function test_click_action_button_to_set_tour_completed() {
  ...
  await SpecialPowers.pushPrefEnv({
    set: [["browser.onboarding.newtour", "private,addons,customize"]],
  });
  ...
  let expectedPrefUpdate = promisePrefUpdated(
    `browser.onboarding.tour.${completedTourId}.completed`,
    true
  );
  await BrowserTestUtils.synthesizeMouseAtCenter(
    `#${completedTourId}-page .onboarding-tour-action-button`,
    {},
...
add_task(async function test_set_watermark_after_all_tour_completed() {
  ...
  await SpecialPowers.pushPrefEnv({
    set: [["browser.onboarding.tour-type", "new"]],
  });
  ...
  TOUR_IDs.forEach(id =>
    Preferences.set(`browser.onboarding.tour.${id}.completed`, true)
  );
  await expectedPrefUpdate;
