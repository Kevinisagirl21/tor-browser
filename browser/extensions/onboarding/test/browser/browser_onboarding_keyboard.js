...
const TOUR_LIST_TEST_DATA = [
  ...
  {
    key: "VK_RETURN",
    expected: { tourId: TOUR_IDs[4], focusedId: TOUR_IDs[4] },
  },
  {
    key: "VK_TAB",
    options: { shiftKey: true },
    expected: { tourId: TOUR_IDs[4], focusedId: TOUR_IDs[3] },
  },
  {
    key: "VK_TAB",
    options: { shiftKey: true },
    expected: { tourId: TOUR_IDs[4], focusedId: TOUR_IDs[2] },
  },
...
const BUTTONS_TEST_DATA = [
  ...
  {
    key: "VK_ESCAPE",
    expected: { focusedId: "onboarding-overlay-button", visible: false },
  },
  { key: "VK_RETURN", expected: { focusedId: TOUR_IDs[1], visible: true } },
  {
    key: "VK_TAB",
    options: { shiftKey: true },
    expected: { focusedId: TOUR_IDs[0], visible: true },
  },
  {
    key: "VK_TAB",
    options: { shiftKey: true },
    expected: { focusedId: "onboarding-overlay-close-btn", visible: true },
  },
  {
    key: " ",
    expected: { focusedId: "onboarding-overlay-button", visible: false },
...
add_task(async function test_tour_list_keyboard_navigation() {
  ...
  await assertOverlayState(tab.linkedBrowser, {
    tourId: TOUR_IDs[0],
    focusedId: TOUR_IDs[0],
  });

  for (let { key, options = {}, expected } of TOUR_LIST_TEST_DATA) {
    info(
      `Pressing ${key} to select ${expected.tourId} and have focus on ${expected.focusedId}`
    );
...
add_task(async function test_buttons_keyboard_navigation() {
  ...
  await assertOverlayState(tab.linkedBrowser, {
    focusedId: "onboarding-overlay-button",
    visible: false,
  });

  for (let { key, options = {}, expected } of BUTTONS_TEST_DATA) {
    info(
      `Pressing ${key} to have ${
        expected.visible ? "visible" : "invisible"
      } overlay and have focus on ${expected.focusedId}`
    );
    await BrowserTestUtils.synthesizeKey(key, options, tab.linkedBrowser);
    await assertOverlayState(tab.linkedBrowser, expected);
  }
