...
(function() {
  ...
  let onCanSetDefaultBrowserInBackground = () => {
    Mozilla.UITour.getConfiguration("appinfo", config => {
      ...
      btn.textContent = canSetInBackGround
  ...
  let onClick = evt => {
    switch (evt.target.id) {
      ...
      case "onboarding-tour-sync-button":
        let emailInput = document.getElementById(
          "onboarding-tour-sync-email-input"
        );
    ...
    if (
      classList.contains("onboarding-tour-item") ||
      classList.contains("onboarding-tour-item-container")
    ) {
  ...
  document
    .getElementById("onboarding-overlay-button")
    .addEventListener("Agent:Destroy", () => Mozilla.UITour.hideHighlight());
  document.addEventListener(
    "Agent:CanSetDefaultBrowserInBackground",
