...
fn test_update_open_invite_and_key_rotation() {
    ...
    // Use it to get a Lox credential
    let (req, state) = open_invite::request(&new_inv);
    let encoded: Vec<u8> = bincode::serialize(&req).unwrap();
    let decoded = bincode::deserialize(&encoded[..]).unwrap();
...
fn test_update_cred() {
    ...
    let (up_req, _upstate) = update_cred::request(&cred, &th.ba.lox_pub).unwrap();
    ...
    let (promreq, _promstate) =
        trust_promotion::request(&cred, &th.ba.lox_pub, th.ba.today()).unwrap();
    let encoded: Vec<u8> = bincode::serialize(&promreq).unwrap();
    let decoded = bincode::deserialize(&encoded[..]).unwrap();
    ...
    let (perf_stat, up_cred) = th.update_cred(&cred);

    // Attempt Update credential with used credential
    let (upreq, _upstate) = update_cred::request(&cred, &th.ba.lox_pub).unwrap();
    ...
    // Attempt to update credential with new keys before next request
    let (up_req, _upstate) = update_cred::request(&cred1, &th.ba.lox_pub).unwrap();
    let encoded: Vec<u8> = bincode::serialize(&up_req).unwrap();
    ...
    assert!(
        promresp.is_ok(),
        "The credential update request should succeed after key rotation"
    );
}
...
fn test_update_invitation() {
    ...
    // Attempt to update invitation with new keys before next request
    let (up_req, _upstate) = update_invite::request(&bob_invite, &th.ba.invitation_pub).unwrap();
    let encoded: Vec<u8> = bincode::serialize(&up_req).unwrap();
    let decoded = bincode::deserialize(&encoded[..]).unwrap();
    ...
    let (upreq, _upstate) = update_invite::request(&bob_invite, &th.ba.invitation_pub).unwrap();
    let encoded: Vec<u8> = bincode::serialize(&upreq).unwrap();
    let decoded = bincode::deserialize(&encoded[..]).unwrap();
    let promresp = th.ba.handle_update_invite(decoded);
    assert!(
